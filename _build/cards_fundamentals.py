...
CARDS = [
    ...
    C("sec-devops", "What is DevOps?", "Culture + automation + ownership",
      ...
      "DevOps rút ngắn đường từ commit đến production ổn định bằng ownership chung, tự động hóa, đo lường và phản hồi nhanh giữa người xây và người vận hành.",
    ...
    C("sec-devops", "CI vs CD", "Precise meanings",
      ...
      "CI: tự động build/test mỗi thay đổi để nhánh chính khỏe. CD thường là Continuous Delivery (luôn sẵn sàng release, có cổng người) hoặc Continuous Deployment (tự lên production).",
      "Nói rõ bạn đang dùng nghĩa CD nào. CI không có test tự động thì chủ yếu là đóng gói.",
    ...
    C("sec-devops", "Pipeline stages mental model", "Commit to verify",
      ...
      "Pipeline điển hình: checkout → cài/build → test → đóng artifact (image/AAB) → deploy env → notify. Secret ngoài Git; prod thường cần approval hoặc tag.",
    ...
    C("sec-devops", "Why multi-environment matters", "dev / staging / prod",
      ...
      "Tách môi trường giảm bán kính nổ: thử ở staging với data/secret staging trước production. Promote có chủ đích — cùng artifact khi được, khác config.",
      "Staging xanh nhưng sai API URL vẫn phá prod. Luôn nêu delta config.",
    ...
    C("sec-linux", "SSH day-to-day", "How you actually manage hosts",
      ...
      "SSH vào Ubuntu/Debian, xem status service, log, disk/memory, áp fix an toàn nhất, verify. Ưu tiên SSH key và user sudo hơn login root bằng mật khẩu.",
      "Vòng có cấu trúc: triệu chứng → tài nguyên → log → thay đổi gần đây → sửa → verify.",
    ...
    C("sec-linux", "Processes, services, permissions", "Just enough OS",
      ...
      "Biết liệt kê process, restart service systemd, đọc quyền rwx. Ưu tiên sửa ownership/group hơn chmod 777. Job dài nên nằm dưới systemd, không nohup quên.",
      "id / ls -l / systemctl / journalctl là tool hằng ngày an toàn khi PV.",
    ...
    C("sec-linux", "Disk and log hygiene", "Classic outage cause",
      ...
      "Disk đầy làm hỏng ghi file, upload CMS, đôi khi SSH cũng lạ. Theo dõi df -h, xoay log, giữ backup khỏi volume app khi được.",
      "Không xóa thư mục database không rõ để lấy chỗ trống.",
    ...
    C("sec-linux", "Package basics on Debian/Ubuntu", "apt awareness",
      ...
      "apt update làm mới index; apt install thêm package; hiểu package khởi động gì (nginx, php-fpm) trước khi upgrade prod mù.",
      "Vá theo lịch; biết rollback upgrade package xấu nếu team có pattern.",
    ...
    C("sec-git", "Branching for delivery", "What CI watches",
      ...
      "Nhánh feature cho việc đang làm; main được bảo vệ để tích hợp; tag cho release candidate. CI thường gắn pattern branch hoặc tag.",
    ...
    C("sec-git", "PR collaboration basics", "Review before build cost",
      ...
      "Pull request bắt config xấu và lộ secret trước khi burn native/cloud build. Diff nhỏ review nhanh hơn mega-PR.",
    ...
    C("sec-git", "What changed? incident habit", "Git as forensics",
      ...
      "Khi production hỏng sau release, so commit xanh cuối với HEAD: code app, Dockerfile, cloudbuild.yaml, template env.",
    ...
    C("sec-git", "Secrets never in Git", "Hard rule",
      ...
      "Keystore, .env có mật khẩu, cloud key và private key không được commit. Dùng Secret Manager / secret CI và .gitignore.",
      "Nếu secret đã commit, phải xoay — chỉ xóa khỏi history chưa đủ nếu đã push.",
    ...
    C("sec-docker", "Why containers for Cloud Run", "Same artifact everywhere",
      ...
      "Container đóng gói app và runtime để staging và Cloud Run chạy cùng bits. Bạn ship image tag/digest, không phải 'chạy được trên laptop'.",
    ...
    C("sec-docker", "Image layers & .dockerignore", "Build speed and size",
      ...
      "Sắp xếp Dockerfile để layer deps được cache. Loại .git và node_modules local bằng .dockerignore. Image nhỏ kéo và start nhanh hơn.",
    ...
    C("sec-docker", "Run locally then deploy", "Parity check",
      ...
      "Chạy image local với cùng PORT và env quan trọng trước khi tin deploy Cloud Run. Local OK nhưng Cloud Run fail → thường env, secret, hoặc nối Cloud SQL.",
      "Local xanh không chứng minh IAM/VPC/SQL đã đúng.",
    ...
    C("sec-docker", "Tagging discipline", "Rollback needs names",
      ...
      "Tag image bằng git sha hoặc build id trên Artifact Registry. Ưu tiên tag/digest bất biến hơn :latest trên production.",
      "Rollback = trỏ Cloud Run về tag/digest tốt đã biết.",
    ...
    C("sec-gcp", "GCP mental model", "Pick the right compute",
      ...
      "Cloud Run: container managed, scale theo request. Compute Engine VM: kiểm soát OS đầy đủ cho web cổ điển (LiteSpeed/Apache/Nginx). Cloud SQL: DB managed. Cloud Storage: file/artifact. Cloud Build: CI build và có thể deploy.",
      "Tôi chọn Run khi app đã container và theo request; VM khi cần kiểm soát web server lâu dài, tùy biến.",
    ...
    C("sec-gcp", "IAM & service accounts", "Who can do what",
      ...
      "User người và service account Cloud Build/Cloud Run cần least privilege: push image, deploy service, đọc secret cụ thể — không cần Owner project cho CI hằng ngày.",
      "Sự cố thường bắt đầu từ 'ai giữ key' và 'SA nào deploy revision này'.",
    ...
    C("sec-gcp", "Cloud Logging & revisions", "Observe what you ship",
      ...
      "Revision Cloud Run + Cloud Logging là mặt debug chính. VM dùng journalctl và error log web server. Luôn gắn lỗi với revision hoặc thời điểm deploy.",
      "Rollback traffic trước; đào root cause sau.",
    ...
    C("sec-gcp", "Storage vs SQL vs secrets", "Put data in the right place",
      ...
      "Cloud Storage cho object/artifact; Cloud SQL cho dữ liệu quan hệ; Secret Manager cho credential. Không để mật khẩu trong object Storage public.",
      "Config app: env không mật OK; credential → secret.",
    ...
    C("sec-web", "Nginx/Apache role", "Reverse proxy & static",
      ...
      "Web server kết thúc HTTP(S), serve static, reverse-proxy hoặc chuyển PHP-FPM/app. Vhost sai → nhầm site, 502, hoặc tải file PHP thay vì chạy.",
    ...
    C("sec-web", "Virtual host basics", "server_name + root",
      ...
      "Mỗi site cần server_name (hoặc ServerName), document root và đường log. App kiểu Laravel thường trỏ root vào public/. TLS gắn cùng vhost sau Certbot.",
    ...
    C("sec-web", "LiteSpeed awareness", "Same concepts, different panel",
      ...
      "LiteSpeed phục vụ nhiều stack PHP/CMS tương tự: vhost, PHP handler, cache, TLS. Thừa kế máy LiteSpeed thì hỏi cùng câu — docroot đâu, error log đâu, PHP chạy thế nào — dù UI admin khác Nginx.",
      "Nói khái niệm trước; không bịa đường click panel không nhớ.",
    ...
    C("sec-web", "502 / 504 quick read", "Upstream vs edge",
      ...
      "502/504 thường là proxy không tới được PHP-FPM/app hoặc upstream timeout. Kiểm socket/port upstream, status service, và fatal app trong log — không chỉ status Nginx.",
    ...
    C("sec-security", "Secrets handling", "Out of Git, least readers",
      ...
      "Giữ credential trong Secret Manager hoặc secret CI. Inject runtime vào Cloud Run hoặc env VM. Hạn chế ai và SA nào được đọc.",
      "Xoay khi nghi lộ; không chỉ xóa tin nhắn chat có mật khẩu.",
    ...
    C("sec-security", "Least privilege", "IAM and OS",
      ...
      "Cấp tối thiểu role để Cloud Build push/deploy và SA app đọc secret/SQL. Trên Linux: tách user sudo và ownership chặt cho web root.",
      "Owner/Editor cho mọi engineer 'cho nhanh' sẽ thành câu chuyện sự cố.",
    ...
    C("sec-security", "SSH & network surface", "Close what you don't need",
      ...
      "SSH chỉ key, VPC/UFW hạn chế cổng (22 thận trọng, 80/443 cho web), không MySQL public. Ưu tiên bastion/IAP nếu team có.",
      "Mỗi cổng mở là giả định bạn phải bảo vệ được.",
    ...
    C("sec-security", "Hardening + docs", "Practical maturity",
      ...
      "Harden vô ích nếu chỉ một người biết server. Ghép control cơ bản (SSH, firewall, secret, vá) với runbook ngắn: bước deploy, chỗ log, renew TLS, rollback.",
      "Tôi nói thẳng: harden thực tế và tài liệu — không nhận chương trình audit chính thức nếu chưa làm.",
